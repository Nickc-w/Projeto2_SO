...
Este projeto implementa uma solução para o clássico problema de Produtor–Consumidor, estudado em Sistemas Operacionais, utilizando threads, semáforos e mecanismos de exclusão mútua em Java.

O objetivo é demonstrar como threads concorrentes podem acessar um recurso compartilhado de forma segura, evitando conflitos, inconsistências e erros como condições de corrida.
...
O produtor pode produzir até 15 itens, enquanto o consumidor pode consumir até 12 itens, respeitando sempre os limites do buffer.
...
🎓 Paralelo com os Conteúdos de Sistemas Operacionais (Item 4)
...
O uso de semáforos reflete os mecanismos clássicos de sincronização utilizados pelo sistema operacional para coordenar o acesso a recursos.

O bloqueio de threads quando o buffer está cheio ou vazio ilustra o conceito de espera e escalonamento, presente tanto em threads de usuário quanto em threads de núcleo.

O uso do mutex (ReentrantLock) representa o conceito de exclusão mútua, fundamental para evitar condições de corrida em ambientes concorrentes.

Dessa forma, o projeto demonstra na prática como o sistema operacional coordena múltiplas threads para garantir execução correta, segura e eficiente.
